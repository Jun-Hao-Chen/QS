...
void printPolynomial(equPointer p) {
    int first = 1;
    while (p) {
...
void insertNode(headPointer eq, int coef, char variable, int expon) {
    if (!eq || coef == 0) return; // 新增檢查，若係數為0則不插入
...
void handleAddition() {
    ...
    if (matched == 3) {
        ...
        printPolynomial(findEquation(resultNotation)->link);
        printf("\n");
        return;
    }
...
void esub() {
    ...
    char resultNotation = '\0', left = '\0', right = '\0';
    int matched = sscanf(input, "%c=%c-%c", &resultNotation, &left, &right);
    if (matched != 3) matched = sscanf(input, "%c-%c", &left, &right);
    ...
    if (matched >= 2) {
        ...
        headPointer negRight = createEquation('-');
        equPointer r = rightEq->link;
        while (r) {
            insertNode(negRight, -r->coef, r->variable, r->expon);
            r = r->link;
        }

        headPointer result = addEquations(leftEq, negRight, (matched == 3 ? resultNotation : '\0'));
        ...
        if (matched == 3) {
            ...
            if (curr) {
                ...
                if (prev) prev->hlink = result;
                else equations = result;
            }
            ...
            printf("%c-%c=", left, right);
            printPolynomial(result->link);
            printf("\n");

            equPointer del = result->link;
            while (del) {
                equPointer temp = del;
                del = del->link;
                free(temp);
